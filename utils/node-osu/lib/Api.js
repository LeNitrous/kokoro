...
const request = require('superagent'),
	BeatmapSet = require('./Base/BeatmapSet.js'),
...
class Api {
	...
	getBeatmapSet(options) {
		return new Promise((resolve, reject) => {
			this.apiCall('/get_beatmaps', options).then(response => {
				if (response.length === 0)
					return this.notFoundAsError ? reject(new Error('Beatmap Set not found')) : resolve(response);
				resolve(new BeatmapSet(response));
			})
		})
	}
